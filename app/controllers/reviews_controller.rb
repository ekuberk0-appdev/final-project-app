class ReviewsController < ApplicationController
  ...
  def create
    ...
    if the_review.valid?
      the_review.save
      redirect_to("/service_providers/#{the_review.service_provider_id}", { :notice => "Review created successfully." })
    else
      redirect_to("/service_providers/#{the_review.service_provider_id}", { :notice => "Review failed to create successfully." })
    end
  end
